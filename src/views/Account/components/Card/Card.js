...
export const YourBalanceCard = () => {
  ...
  const balance = useAppSelector(state => {
    return state.account && state.account.balances.kami;
  });
  return (
    <Card className="nested-card">
      <Card.Title>Your Balance</Card.Title>
      <Card.Value>{!isAppLoading && !isAccountLoading && formatCurrency(balance * marketPrice, 2)}</Card.Value>
      <Card.Secondary>{formatNumber(balance, 2)} KAMI</Card.Secondary>
    </Card>
  );
};
...
export const YourEarningsCard = () => {
  ...
  return (
    <Card className="nested-card green">
      ...
      <Card.Secondary>{formatNumber(balance * 0.020999, 2)} KAMI</Card.Secondary>
    </Card>
  );
};
...
export const NextRewardAmountCard = () => {
  ...
  return (
    <Card className="long-card">
      <Card.Right>{!isAccountLoading && formatNumber(balance * 0.04189, 2)} KAMI</Card.Right>
...
export const NextRewardAmountUSDCard = () => {
  ...
  return (
    <Card className="long-card green">
      <Card.Right>
        {!isAppLoading && !isAccountLoading && formatCurrency(balance * marketPrice * 0.0004189, 2)} USD
      </Card.Right>
